"""Cognitive Behavioral Therapy (CBT) Models for MyBella"""
...
class CBTSession(db.Model):
    ...
    persona = db.Column(db.String(50), default='Maya')  # Legacy field
    ...
    persona_profile = db.relationship('PersonaProfile', backref='cbt_sessions', foreign_keys=[persona_id])
...
class EmotionMatch(db.Model):
    ...
    persona = db.relationship('PersonaProfile', backref='emotion_matches', foreign_keys=[persona_id])
...
class DailyNote(db.Model):
    ...
    persona = db.relationship('PersonaProfile', backref='daily_notes', foreign_keys=[persona_id])
...
class CheckInEntry(db.Model):
    ...
    persona_used = db.Column(db.String(50), default='Isabella')  # Legacy field
    ai_message = db.Column(db.Text, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    user = db.relationship('User', backref='checkin_entries')
    persona = db.relationship('PersonaProfile', backref='checkin_entries', foreign_keys=[persona_id])


class SecretVaultEntry(db.Model):
    """PIN-protected private journal entries"""
    __tablename__ = 'secret_vault_entries'
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    title = db.Column(db.String(200), nullable=False)
    content = db.Column(db.Text, nullable=False)
    pin_hash = db.Column(db.String(64), nullable=False)  # SHA-256 hash of PIN
    tags = db.Column(db.Text, nullable=True)  # JSON array
    mood = db.Column(db.String(50), nullable=True)
    is_encrypted = db.Column(db.Boolean, default=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    user = db.relationship('User', backref='vault_entries')
